...
public class JavaTutorial36 {
	...
	//ResultSetMetaData contains information on the data returned to the query
	static ResultSetMetaData metaData;
	
	//DefaultTableModel defines the methods JTable will use
	//Overriding the getColumnClass
	...
	public static void main(String[] args) {
		...
		try{
			//The driver allows you to query the database with Java forName dynamically
			//loads the class for you
			...
			conn = DriverManager.getConnection("jdbc:mysql://localhost/lahman2016", "root", "AEROspace2014////");
			...
			"((b.H+b.BB)+(2.4*(b.AB+b.BB)))*(t.TB+(3*(b.AB+b.BB)))/(9*(b.AB+b.BB))-(0.9*(b.AB+b.BB)) AS TTRC, " +
			"b.teamID AS Team, s.salary AS Salary, " +
			"CAST( s.salary/(((b.H+b.BB)+(2.4*(b.AB+b.BB)))*(t.TB+(3*(b.AB+b.BB)))/(9*b.AB+b.BB))-(0.9*(b.AB+b.BB)) as decimal(10,2)) AS CPR, " + 
			...
			"AND s.yearID = b.yearID AND b.yearID AND b.AB > 50 AND b.playerID = f.playerID " +
			 ...
			 * String selectStuff = "select b.yearID, b.playerID, " + 
			 * "CONCAT(m.nameFirst, ' ', m.nameLast) AS Name, " +
			 * "((b.H+b.BB)+(2.4*(b.AB+b.BB)))*(t.TB+(3*(b.AB+b.BB)))/(9*
			...
			while(rows.next()) {
				...
				tempRow = new Object[] {rows.getInt(1), rows.getString(2), rows.getString(3), rows.getDouble(4), rows.getString(5),
