...
//Thrown when a URL doesn't contain http:// and other rules like that
import java.net.MalformedURLException;
import java.net.URL;

//A text component that allows for rich text and basic html display
...
//Provides information on events triggered through interaction with links
...
public class JavaTutorial39 extends JFrame implements HyperlinkListener, ActionListener {
	
	public static void main(String[] args) {
		new JavaTutorial39("file:///Code/Java/Java Youtube/Java_Tutorial_39/src/htmlexample.html");
	}
	...
	public JavaTutorial39(String defaultURL)  {
		JFrame frame = new JFrame("Java Browser");
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
		this.defaultURL = defaultURL;
		
		//if the user interacts with the JTextField the actionPerformed method is called
		...
		try {
			...
			//Ex. Click on a link change the JEditorPane page location
			htmlPage.addHyperlinkListener(this);
			
			//You can leave this true for rich text documents but it will mess up web page display
		...
		//if something goes wrong with locating the html page this will handle error
		catch(IOException e) {
			e.printStackTrace();
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == theURL) {
			...
			JOptionPane.showMessageDialog(JavaTutorial39.this, "Please Enter a Web Address", "Error", JOptionPane.ERROR_MESSAGE);
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(JavaTutorial39.this, "Please use http://", "Error", JOptionPane.ERROR_MESSAGE);		
		}
		catch(IOException e1) {
			e1.printStackTrace();
		}
